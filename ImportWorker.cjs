...
async function startTask() {
  ...
  try {
    ...
    if (filePath.endsWith(".csv")) {
      ...
      import_worker_threads.parentPort?.postMessage({ type: "progress", pct: 60, stage: "Creating schema..." });
      try {
        db.run(`
        ...
        import_worker_threads.parentPort?.postMessage({ type: "error", error: createErr.message });
        return;
      }
      import_worker_threads.parentPort?.postMessage({ type: "progress", pct: 75, stage: "Loading JSON/CSV..." });
      const Papa = require_papaparse();
      ...
      for (let i = startRow - 1; i < rows.length; i++) {
        ...
        inserted++;
        if (inserted % 1e4 === 0) {
          import_worker_threads.parentPort?.postMessage({ type: "progress", pct: Math.min(90, 75 + inserted / rows.length * 15), stage: `Parsing ${inserted} rows...` });
        }
      }
      stmt.free();
      ...
      import_worker_threads.parentPort?.postMessage({ type: "done", rowCount: inserted, columns: keys });
    } else if (filePath.endsWith(".xlsx")) {
      const ExcelJS = require("exceljs");
